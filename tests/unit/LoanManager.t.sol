...
//         uint256 amount = 1e7 * 1e6;
//         _investAssets(USDT, address(usdtPool), amount);
//         uint256 wmInitialBal = usdtPool.balanceOf(address(withdrawalManagerUSDT));
//         // time warp
//         vm.warp(block.timestamp + 2 weeks);
...
//         vm.expectRevert("LM: Not in Window");
//         loanManager.redeem(address(usdt));
//         assertEq(usdtPool.balanceOf(address(withdrawalManagerUSDT)), wmInitialBal + lmUSDT / 10 ** 12);

//         //removing shares
//         loanManager.remove(address(usdt));
//         assertEq(usdtPool.balanceOf(address(loanManager)), lmUSDT / 10 ** 12);
//         assertEq(usdtPool.balanceOf(address(withdrawalManagerUSDT)), wmInitialBal);
