...
//         LoanManager imp1 = new LoanManager(address(pool));
...
contract TestDeposit is BaseTest {
    ...
    //     // Constraint input amount
    //     vm.assume(amount < _getUpperBoundDeposit(address(pool), address(poolManagerUSDC)));
    //     uint256 shares = usdcPool.previewDeposit(amount);
    //     vm.assume(shares == 0);
...
contract TestRequestRedeem is BaseTest {
    ...
    function test_requestRedeem_USDC() external {
        ...
        console.log("LUSDC issued", lmUSDC, usdcPool.decimals());
        ...
        assertEq(usdcPool.balanceOf(address(usdcPool)), lmUSDC/10**12);
        vm.stopPrank();
    }
...
contract TestRedeem is BaseTest {
    ...
    function test_redeem_USDC() external {
        ...
        console.log(stablesRedeemed, expectedUSDC, IERC20(USDC).balanceOf(NSTBL_HUB), "check stables received");
